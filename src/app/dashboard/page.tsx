...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}payments/transactions`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="pt-28 pb-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-gray-900">Welcome, {user?.first_name} 👋</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          {[
            ...
            <div
              key={index}
              className="bg-white rounded-xl p-6 shadow-lg transform transition-all duration-300 hover:translate-y-[-5px] hover:shadow-xl border border-gray-100"
            >
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xs text-green-500">
                    {investmentData?.percentage_invested_in_rental_assests?.toLocaleString() || 0}%
                  </span>
                </div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                  <div
                    ...
                    style={{ width: `${investmentData?.percentage_invested_in_rental_assests || 0}%` }}
                  ></div>
                </div>
              </div>
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xs text-green-500">
                    {investmentData?.percentage_invested_in_non_rental_assests?.toLocaleString() || 0}%
                  </span>
                </div>
                <p className="text-xl font-bold text-gray-900">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                  <div
                    ...
                    style={{ width: `${investmentData?.percentage_invested_in_non_rental_assests || 0}%` }}
                  ></div>
                </div>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-bold text-gray-900">
                      ₹{property.investment_amount?.toLocaleString() || 0}
                    </p>
                  </div>
                </div>
              ))}
              ...
              {transactions.length > 0 ? (
                transactions.slice(0, 3).map((tx, index) => (
                  <div key={index} className="flex items-center p-4 bg-gray-50 rounded-lg">
                    <div className="flex-shrink-0 p-3 bg-gray-100 rounded-lg mr-4">
                      <CreditCard className={`h-5 w-5 ${tx.type === "DEPOSIT" ? "text-green-500" : "text-red-500"}`} />
                    </div>
                    <div className="flex-grow">
